...
!function($) {
	...
	$.fn.multiselectable = function(options) {
		...
		function mouseDown(e) {
			...
			var item = $(this),
				...
				//remove class selecionada ------------------------------------------------
				id = parent.attr('id');
				removeClassDragDrop(id);
	...
	$.fn.multisortable = function($scope, scope1, scope2, options) {
		if (!options) {
			options = {}
		}
		...
		return this.each(function() {
			...
			options.receive = function(event, ui) {
				...
				var listScope = $(ui.item.closest('ul')).attr('id');

				//listScope2 = (listScope == scope1) ? scope2 : scope1;
				//$scope.tempDrapDrop = [];
				//var log = '{';
				var listScope2 = (listScope != scope1) ? scope1 : scope2; 

				$scope.dragDrop(listScope, listScope2);
